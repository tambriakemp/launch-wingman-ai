-- Fix encrypt_token function with proper bytea casting for digest()
...
BEGIN
  ...
  -- Cast to bytea for digest function compatibility
  encryption_key := encode(digest(('launchely_' || current_database() || '_token_key_v1')::bytea, 'sha256'), 'hex');
...
-- Fix decrypt_token function with proper bytea casting for digest()
