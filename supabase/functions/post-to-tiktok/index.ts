...
async function refreshTikTokToken(supabase: any, userId: string, refreshToken: string): Promise<string | null> {
  ...
  const { data: encryptedAccess } = await supabase
    .rpc("encrypt_token", { token: data.access_token });

  const { data: encryptedRefresh } = await supabase
    .rpc("encrypt_token", { token: data.refresh_token });
...
serve(async (req) => {
  ...
  try {
    ...
    if (tokenExpiry <= new Date()) {
      ...
      const newToken = await refreshTikTokToken(supabase, user.id, connection.refresh_token);
      if (!newToken) {
        return new Response(
