...
interface SavedIdeasSheetProps {
  open: boolean;
...
export const SavedIdeasSheet = ({ 
  open, 
  onOpenChange, 
  projectId, 
  onOpenItem 
}: SavedIdeasSheetProps) => {
  const { user } = useAuth();
  ...
  return (
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent className="w-full sm:max-w-md">
        <SheetHeader>
          <SheetTitle>Saved ideas & drafts</SheetTitle>
          <SheetDescription>
            Click to view or edit. Here when you're ready.
          </SheetDescription>
        </SheetHeader>
        
        <ScrollArea className="h-[calc(100vh-8rem)] mt-6 -mx-6 px-6">
          {allItems.length === 0 ? (
            <div className="text-center py-12">
              <FileText className="w-10 h-10 text-muted-foreground/30 mx-auto mb-3" />
              <p className="text-sm text-muted-foreground">
                Nothing saved yet. Save ideas from the main page to see them here.
              </p>
            </div>
          ) : (
